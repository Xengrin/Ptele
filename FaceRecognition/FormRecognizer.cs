...
namespace FaceRecognition
{
	public partial class FormRecognizer : Form
	{
		...
		private void ProcessFrame(object sender, EventArgs arg)
		{
			...
			{
				facesDetected[i].X += (int)(facesDetected[i].Height * 0.1); //0.15
				...
				frame.Draw(facesDetected[i], new Bgr(Color.Green), 2);

				if (recognizer.IsTrained)
				{
					var result = recognizer.Recognize(currentFaceGray);
					frame.Draw(result.Name + ", Na : " + (int)result.Distance+"%",
						new Point(facesDetected[i].X , facesDetected[i].Y ),
        ...
        private void FormRecognizer_Load(object sender, EventArgs e)
        {
